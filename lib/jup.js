...
;(function() {
    ...
    var JUP = namespace.JUP = (function() {
        var Util = {
            ...
            translate: function(markup, data) {

                var toString = Object.prototype.toString, c = [], atts = [],
                        selfClosing = false;

                for (var i = 0, l = markup.length; i < l; i++) {
                    if (markup.hasOwnProperty(i)) {
                        if (markup[i] && typeof markup[i] === "object") {
                            if (toString.call(markup[i]) !== "[object Array]") {
                                for (var attribute in markup[i]) {
                                    if (markup[i].hasOwnProperty(attribute)) {
                                        atts.push([" ", this.replace(attribute, data), "=\"", this.replace(markup[i][attribute], data), "\""].join(""));
                                    }
                                }
                                c[i] = "";
                                c[0] = [c[0], atts.join("")].join("");
                            } else {
                                c[i] = this.translate(markup[i], data);
                            }
                        } else {
                            c[i] = this.replace(markup[i], data);
                        }
                        if (typeof c[0] === "string") {
                            ...
                            c[0] = ["<", markup[0], atts.join(""), (selfClosing ? "/>" : ">")].join("");
